...
console.log('🔑 OpenRouter API Key configured:', OPENROUTER_API_KEY ? 'Yes (sk-or-v1-...)' : 'No');
console.log('🔑 Google Gemini API Key configured:', GOOGLE_API_KEY ? 'Yes (AQ.Ab...)' : 'No');
console.log('📧 Apps Script URL configured:', APPS_SCRIPT_URL ? 'Yes' : 'No');
...
// Key Rotation Orchestrator: 1st OpenRouter -> 2nd Google Gemini
async function handleChatWithKeyRotation(history, message, patientContext) {
  // Build system prompt with live patient context
...
══════════════════════════════════════════════════════════════`;
  ...
  try {
    ...
    console.warn(`[SmartQueue AI] ⚠️ OpenRouter failed (${openRouterErr.message}). Switching to Google Gemini API (Rotation Step 2)...`);
  }
...
// ── ENDPOINT HANDLERS (Vercel & Local Shared) ────────────────────────
function handleHealthRequest(req, res) {
  ...
  const geminiKey = process.env.GOOGLE_API_KEY || GOOGLE_API_KEY;
...
async function handleChatRequest(req, res) {
  ...
  try {
    ...
    if (!message && history.length === 0) {
      ...
      return res.end(JSON.stringify({ error: 'Message or history is required' }));
    }
...
async function handleSendConfirmationRequest(req, res) {
  ...
  try {
    ...
    if (!patientEmail && !doctorEmail) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ success: false, error: 'At least one email (patient or doctor) is required' }));
    }

    const scriptUrl = process.env.APPS_SCRIPT_URL || APPS_SCRIPT_URL;
    if (!scriptUrl) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ success: false, error: 'APPS_SCRIPT_URL not configured in .env' }));
    }
    ...
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 30000); // 30s timeout for Apps Script
    ...
    // Google Apps Script doPost requires text/plain and redirect: 'follow'
    ...
    // Parse response — may be JSON or HTML
    ...
    if (result.success || scriptRes.ok) {
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ success: false, error: result.error || 'Apps Script error' }));
    }
  } catch (err) {
...
// ── HTTP SERVER (Local Node.js Development) ─────────────────────────
const server = http.createServer(async (req, res) => {
  ...
  // Security check: must remain inside workspace
...
if (require.main === module) {
  server.listen(PORT, () => {
    console.log(`\n======================================================`);
    console.log(`🚀 SmartQueue AI Server running at: http://localhost:${PORT}`);
